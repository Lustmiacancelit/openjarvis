...
export function UpdateChecker() {
  ...
  const checkForUpdate = useCallback(async () => {
    ...
    try {
      ...
      // Silently ignore â€” likely running in browser or no update available
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // VITE_Jarvis_NO_UPDATER=1`` before ``npm run tauri dev`` to
    // silence the 30-min poll. See docs/desktop-auto-update.md.
    const noUpdater = (import.meta as any).env?.VITE_Jarvis_NO_UPDATER;
